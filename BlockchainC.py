...
import json #to format the blocks#
from time import time #for each block's timestamp#


class Blockchain(object):
    def __init__(self):
        self.chain = [] #chain is the empty list that we'll add blocks to#
        self.pending_transactions = [] # when users are sending coins to eachother, their transactions will sit in this array

        self.new_block(previous_hash="The Times 03/Jan/2009 Chancellor on brink of second bailout for banks.",
                       proof=100) #used to add each block to the chain

#Now we have an empty chain and we will add blocks to it
